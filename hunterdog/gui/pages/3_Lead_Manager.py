...
if selected_indexes:
    ...
    with batch_left:
        ...
        if st.button("应用状态"):
            selected_names = {visible[index].get("name") for index in selected_indexes}
            for lead in leads:
                if lead.get("name") in selected_names:
    ...
    with batch_right:
        if st.button("删除选中"):
            selected_names = {visible[index].get("name") for index in selected_indexes}
            write_leads([lead for lead in leads if lead.get("name") not in selected_names])
            st.rerun()
